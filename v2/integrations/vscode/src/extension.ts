...
function getConfig() {
  ...
  return {
    ...
    notionServerUrl: cfg.get<string>('notionServerUrl', cfg.get<string>('serverUrl', 'http://localhost:8080')),
  };
}
...
function createNotionReportCommand(diag: vscode.Diagnostic): vscode.CodeAction {
...
class RakshakTreeProvider implements vscode.TreeDataProvider<FindingTreeItem> {
  private _onDidChangeTreeData = new vscode.EventEmitter<FindingTreeItem | undefined>();
  readonly onDidChangeTreeData = this._onDidChangeTreeData.event;
  
  refresh(): void { this._onDidChangeTreeData.fire(undefined); }
  getTreeItem(el: FindingTreeItem): vscode.TreeItem { return el; }
  ...
  private getSeverityGroups(): FindingTreeItem[] {
    ...
    if (total === 0) {
      const welcome = new FindingTreeItem(
...
class RakshakSidebarProvider implements vscode.WebviewViewProvider {
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider('rakshak-sidebar', sidebarProvider)
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('rakshakai.scanFile', async () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('rakshakai.showFindingDetails', (uri: vscode.Uri, diag: vscode.Diagnostic) => {
      const f = findingsCache.get(uri.toString());
      if (!f) {
      ...
      const panel = vscode.window.createWebviewPanel(
        'rakshakai-details', `🛡️ ${f.vulnerability}`,
        vscode.ViewColumn.Beside, { enableScripts: false }
      );
...
<body>
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('rakshakai.notionReport', async (diag: vscode.Diagnostic) => {
      const cfg = getConfig();
      const ed = vscode.window.activeTextEditor;
      const docUri = ed?.document.uri.toString() || '';
      const f = findingsCache.get(docUri) || (diag as any);
      ...
      try {
        const r = await axios.post(`${cfg.notionServerUrl}/v2/notion/report`, payload, { timeout: 15_000 });
        if (r.data.queued) {
          vscode.window.showWarningMessage(
            `Notion is unavailable; the report was queued (${r.data.queue_size} pending).`
          );
          return;
        }
        ...
        vscode.window.showInformationMessage(
          ...
          if (choice && url) vscode.env.openExternal(vscode.Uri.parse(url));
        });
      } catch (e: any) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('rakshakai.setupPrecommit', async () => {
...
`;
      ...
      try {
        ...
        vscode.window.showInformationMessage(
          '✅ Git pre-commit hook installed! Commits will be blocked if vulnerabilities are found.',
          'Test Hook',
          'Disable Hook'
        ).then(choice => {
          if (choice === 'Test Hook') {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('rakshakai.testPrecommit', async () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('rakshakai.exportToNotion', async () => {
      const cfg = getConfig();
      const findings: { file: string; finding: Finding }[] = [];
      ...
      vscode.window.withProgress({
        ...
        try {
          ...
          const r = await axios.post(`${cfg.notionServerUrl}/v2/notion/export-batch`, payload, { timeout: 30_000 });
          
          progress.report({ increment: 100 });
          
          vscode.window.showInformationMessage(
            `✅ Created ${r.data.created} complete Notion report${r.data.created === 1 ? '' : 's'}`,
            'Open Notion'
          ).then(choice => {
